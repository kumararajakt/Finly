...
describe('RulesService', () => {
  ...
  it('creates a rule with defaults', async () => {
    ...
    await expect(
      service.create(USER_ID, { whenText: ' x ', thenText: ' y ' }),
    ).resolves.toBe(row);
  });
  ...
  it('updates an existing rule', async () => {
    ...
    await expect(
      service.update(USER_ID, 'r1', { enabled: false }),
  ...
  it('removes an existing rule', async () => {
    ...
    await expect(service.remove(USER_ID, 'r1')).resolves.toBeUndefined();
  });

  it('throws not found when removing a missing rule', async () => {
    db.delete.mockReturnValue(deleteChain([]));
    await expect(service.remove(USER_ID, 'r1')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
});
